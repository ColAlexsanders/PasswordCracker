...
fn compare_passwords_argon2(word: &str, target_hash: &PasswordHash) -> bool {
    Argon2::default().verify_password(word.as_bytes(), target_hash).is_ok()
}
...
fn read_target_hash_argon2(prompt: &str) -> io::Result<String> {
    loop{
        print!("{}", prompt);
        io::stdout().flush()?;
        let mut input = String::new(); 
...
/*    println!("3. Crack zip file with dictionary attack");
    ...
    match input.trim().parse::<usize>() {
        ...
        _ => {
            eprintln!("invalid choice. please enter a number between 1 and 3 or the exit option.");
            display_menu()
        }
    }
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {    
    loop{
        ...
        match choice {
            ...
            3 => {
                target_hash = read_target_hash_argon2("Enter target hash:")?;
                passwords = match read_from_list(file_path) {
                    Ok(p) => p,
                    Err(e) => {
                        eprintln!("Error reading wordlist: {}", e);
        ...
        for i in 0..num_threads {
            ...
            let handle = thread::spawn(move || {
                let target_hash_parsed_argon2= if choice_clone== 3 {
                    match PasswordHash::new(&target_hash_clone) {
                        Ok(parsed) => {
                            Some(parsed)
                        },
                        Err(e) => {
                            println!("Thread {}: Invalid Argon2 hash format: {}", i, e);
                            return;
                        }
                    }
                } else {
                ...
                for word in &words {
                    ...
                    match choice_clone {
                        ...
                        3 => {
                            if let Some(ref parsed_hash)= target_hash_parsed_argon2 {
                                 if compare_passwords_argon2(word, parsed_hash) {
                                    println!("Found password: {}", word);
                                    found_password_clone.store(true, Ordering::SeqCst);
                                }
                            }
